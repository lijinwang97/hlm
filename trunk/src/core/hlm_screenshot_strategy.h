...
class HlmScreenshotStrategy {
   public:
    ...
    virtual shared_ptr<HlmTask> createTask(const string& streamUrl, const string& outputDir, const string& filenamePrefix, const json::rvalue& body) = 0;
};

// 按时间间隔截图策略
class HlmIntervalScreenshotStrategy : public HlmScreenshotStrategy {
   public:
    shared_ptr<HlmTask> createTask(const string& streamUrl, const string& outputDir, const string& filenamePrefix, const json::rvalue& body) override;
};
